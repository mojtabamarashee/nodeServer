    ...
    instAll.forEach(async (v, i) => {
      ...
      if (ind != -1) {
        ...
        if (v.name.match(/^([^0-9]*)$/) && !allRows[ind].pClosingHist) {
          let url =
            'http://cdn.tsetmc.com/Loader.aspx?ParTree=15131P&i=' +
            v.inscode +
            '&d=' +
            day; //20200223
          ...
          axios
            ...
            .then(response => {
              ...
              if (pClosingHist1DayRecvCntr == pClosingHist1DaySendCntr) {
                console.log(
                  'pClosing1DaySendCntr = ',
                  pClosingHist1DaySendCntr,
                );
                console.log(
                  'pClosing1DayRecvCntr = ',
                  pClosingHist1DayRecvCntr,
                );
                res(1);
              }
    ...
    instAll.forEach(async (v, i) => {
      ...
      if (ind != -1) {
        if (v.name.match(/^([^0-9]*)$/) && !allRows[ind].intraDayPrice) {
          let url =
            'http://www.tsetmc.com/tsev2/chart/data/IntraDayPrice.aspx?i=' +
            v.inscode;
          ...
          axios
            ...
            .then(async response => {
              ...
              var row = await dbo
                ...
                .updateOne(
                  {name: v.name},
                  {$set: {intraDayPrice: intraDayPrice}},
                );
    ...
    .forEach((v, i) => {
      ...
      axios
        ...
        .then(response => {
          ...
          t == -1
            ? (allRows.push({pl: pClosing, inscode: v.inscode, name: v.name}),
              console.log(pClosing))
            : null;
...
function GetKhodro() {
  let date = [],
    value = [];
  url =
    'http://www.tsetmc.com/tsev2/chart/data/Index.aspx?i=20213770409093165&t=value';
  axios
    .get(url, {
      headers: typicalHeader,
    })
