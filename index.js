...
function ConnectToDB() {
	let db = MongoClient.connect(
		...
		(e, db) => {
			...
			instAll.filter((v, i) => i < 100).forEach((v, i) => {
	...
	axios
		...
		.then(response => {
			response.data.split(';').map((v, i) => {
				...
				allRows[i].date = date;
				dbo.collection('allRows').insertOne(allRows[i], () => console.log(testCntr++));
				error = 0;
			});
		})
		.catch(error => {
...
function GetPClosingHist(dbo, id) {
	instAll.forEach((v, i) => {
		if (v.name.match(/^([^0-9]*)$/) /*&& !v.pClosingHist*/) {
			...
			axios
				...
				.then(response => {
					...
					var row = dbo
						.collection('allRows')
						.updateOne({name: v.name}, {$set: {vHist: vHist, pClosingHist: pClosingHist}});
				})
				.catch(error => {
					histRecvCntr++;
					if (histRecvCntr == histSendCntr) {
